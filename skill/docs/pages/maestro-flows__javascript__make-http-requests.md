> For the complete documentation index, see [llms.txt](https://docs.maestro.dev/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.maestro.dev/maestro-flows/javascript/make-http-requests.md).
...
The following script sends a request to an appointments API and stores the `appointmentTitle` in the [`output` object](/maestro-flows/javascript/manage-data-and-states.md#the-output-object) to be used by the main flow.
...
Learn how to [generate synthetic test](/maestro-flows/javascript/generate-synthetic-data.md) data to create dynamic, realistic data at runtime, making your tests more flexible, reusable, and less dependent on hard-coded values.
...
```
GET https://docs.maestro.dev/maestro-flows/javascript/make-http-requests.md?ask=<question>&goal=<endgoal>
```
