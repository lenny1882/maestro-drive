> For the complete documentation index, see [llms.txt](https://docs.maestro.dev/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.maestro.dev/maestro-flows/flow-control-and-logic/specify-and-start-devices.md).
...
Maestro allows you to create and launch Android emulators or iOS simulators directly from the CLI. These devices are configured to approximate the environment hosted on [Maestro Cloud](/maestro-cloud/readme.md), ensuring your Flows are compatible when you scale up.
...
When sharding, you're using the same workspace on multiple devices at the same time. Taking a screenshot with the same name will cause overwriting. Look at the [available environment variables](/maestro-flows/flow-control-and-logic/parameters-and-constants.md#built-in-parameters) to differentiate.
...
* [Maestro CLI commands and options](/maestro-cli/maestro-cli-commands-and-options.md): Full list of available flags and commands.
* [Test reports and artifacts](/maestro-flows/workspace-management/test-reports-and-artifacts.md): Learn how reports are generated when running in parallel.
...
```
GET https://docs.maestro.dev/maestro-flows/flow-control-and-logic/specify-and-start-devices.md?ask=<question>&goal=<endgoal>
```
