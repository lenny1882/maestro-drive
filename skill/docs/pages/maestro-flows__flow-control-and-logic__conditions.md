> For the complete documentation index, see [llms.txt](https://docs.maestro.dev/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.maestro.dev/maestro-flows/flow-control-and-logic/conditions.md).
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `visible`    | Executed if the element matching the selector is visible. The element must be defined using one or more [Selectors](/reference/selectors.md).          |
| `notVisible` | Executed if the element matching the selector is **not** visible. The element must be defined using one or more  [Selectors](/reference/selectors.md). |
...
This is the most idiomatically expressive way to handle conditions. It clearly defines the intent "Only run these commands *when* this condition is met." You combine [`runFlow`](/reference/commands-available/runflow.md) and `when`:&#x20;
...
Now that you understand how to use conditions in your Flows, learn how to use [parameters and constants](/maestro-flows/flow-control-and-logic/parameters-and-constants.md) or explore all the possibilities of using [JavaScript](/maestro-flows/javascript/javascript-overview.md) to create tests.
...
```
GET https://docs.maestro.dev/maestro-flows/flow-control-and-logic/conditions.md?ask=<question>&goal=<endgoal>
```
