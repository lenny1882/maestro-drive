> For the complete documentation index, see [llms.txt](https://docs.maestro.dev/llms.txt). Markdown versions of documentation pages are available by appending `.md` to page URLs; this page is available as [Markdown](https://docs.maestro.dev/maestro-flows/flow-control-and-logic/parameters-and-constants.md).
...
| `MAESTRO_SHARD_ID`    | Identifies the shard this test is running on. Starts counting at 1, and defaults to 1 when not using [sharding](/maestro-flows/flow-control-and-logic/specify-and-start-devices.md). |
| `MAESTRO_SHARD_INDEX` | Identifies the shard this test is running on. Starts counting at 0, and defaults to 0 when not using [sharding](/maestro-flows/flow-control-and-logic/specify-and-start-devices.md). |

### Next steps

To continue learning how to create Flows, check the following pages:

* [Nested flows](/maestro-flows/flow-control-and-logic/nested-flows.md): Learn more about using `runFlow` to pass environment variables.
* [Maestro CLI overview](/maestro-cli/readme.md) : Explore more options for the `-e` flag.
* [JavaScript overview](/maestro-flows/javascript/javascript-overview.md): Learn how to manipulate parameters using JavaScript logic.
...
```
GET https://docs.maestro.dev/maestro-flows/flow-control-and-logic/parameters-and-constants.md?ask=<question>&goal=<endgoal>
```
