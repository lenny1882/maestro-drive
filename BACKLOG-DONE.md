...
## 88. A simulator booted by a session that has ended stays booted for ever — **DONE 18 Sep 2026**
...
**Built the same day: `drivers.sh rig reap`.** Piece 1 and piece 2 of the fix, as
raised. The three tests are the label reclaim's, applied to the boot rather than
the name — nobody claims it, no driver is live on it, and nothing has written to
it since a previous calendar day — and it **lists and stops**. `rig reap
--shutdown` is a second command, because item 74's lesson was a session that
killed four working simulators to recover from a boot storm it had misread.

**Piece 2's number is the calendar day**, which is what the wall already uses for
a name and what item 67's piece 4 chose for the same reason: a device driven at
23:00 is yesterday's work by breakfast, though it is only ten hours old.
`LABEL_STALE_AFTER`'s hour was never a candidate for a boot.

**The freshness signal took two goes.** The first version read the newest app
container directory, which reported 16 Sep for a device that had been driven all
day on the 18th: a container's mtime only moves when its immediate contents
change. Going one level deeper — `Documents/`, `Library/` — gave 18 Sep 12:56,
exactly what a recursive `find` over the whole container returned, for the cost
of a glob.

**Seven cases**, calling the function with the three things it asks the Mac for
stubbed. Each of the three tests is asserted to keep a device on its own, and one
case fixes the calendar-day rule against an hour count by putting a device's last
use at 23:00 yesterday. Two of the seven failed first for reasons worth keeping:
`_rig_reap` declares `local claimed`, and bash scopes dynamically, so a stub
reading `$claimed` from inside the call saw the function's empty local rather
than the test's; and the claim ledger is one udid per line, which `grep -qx` is
strict about on purpose.

**Piece 3 — saying it on the wall — is not built**, and is still the judgement
call item 67's piece 4 left open.
